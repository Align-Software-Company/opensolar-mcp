...
- [x] Binding `0.0.0.0` without `MCP_HTTP_ALLOWED_HOSTS` fails closed; invalid `OPENSOLAR_READ_ONLY` / `MCP_TRANSPORT` values and an empty HTTP host are also rejected.
...
- [x] Binary private files and system images are not duplicated as base64 in `structuredContent`; file/image reads enforce the 10 MB ceiling while streaming, including without Content-Length, and private-file redirects cannot downgrade to non-HTTPS.
