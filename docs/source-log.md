...
| Project share preflight | `preflight_project_share` was not called on a live org. The sharing-a-project page shows `shared_with` with `org_id` and optional `is_shared`. The write default of `is_shared` is not used as a read contract. The payment option, module activation, and systems list examples do not show which orgs an entity is shared with. The bulk share page says unshared entities add warnings to project data and does not name the warning key. Inverter, battery, and other activation id keys are not in the systems list example. No response body was stored. |
