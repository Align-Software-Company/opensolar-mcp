...
| 48389 | 2026-09-22 | A disposable contact received a PUT that sent only a changed `phone`. The following GET kept `first_name`, `family_name`, and `email`. The fixture was deleted. No response body stored. |
| 48389 | 2026-09-22 | MCP `search_contacts` and `search_projects` returned a listed record id. Scan metadata matched the pages read. An incomplete scan was not treated as absence. No response body stored. |
| 48389 | 2026-09-22 | MCP `get_project_snapshot` returned the project id from the documented project read. A forced workflow failure became a gap and left the project id in place. The output had no design blob, signed file URL, `configuration_json`, or credential field. No response body stored. |
| 48389 | 2026-09-22 | MCP `compare_project_systems` returned system ids from the systems list for one project. The details call ran once only when a listed system was missing a hardware group. No response body stored. |
| 48389 | 2026-09-22 | MCP `get_project_design` ran because this org has Raw Data. Summary matched the decoded structure. `components` and `energy` stayed `unmapped: true`. Geometry did not return coordinates. The raw design payload was not stored. |
...
| Project list fields used by `search_projects` | Projects page retrieved 2026-09-22. The list example includes `title`, `address`, `business_name`, and `contacts_data` with `email`, `phone`, `first_name`, `family_name`, and `display`. Project `identifier`, `locality`, `state`, and `zip` are on the detail example, not the list example. Org 48389 called the MCP tool the same day and a listed project id was returned. No response body was stored. |
| Project snapshot fields | The same projects page shows detail fields `payment_option_sold`, `private_files_data`, `shared_with`, `usage`, `events_data`, and `assigned_role_data`. `get_project_snapshot` uses those. Org 48389 called the MCP tool on 2026-09-22. The project id matched, and a forced workflow failure became a gap. No response body was stored. |
