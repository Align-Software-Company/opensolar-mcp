...
| Accessing shared projects and entities | https://developers.opensolar.com/api/accessing-shared-projects-and-entities/ | 2026-09-22 |
...
Recorded from prior live-verification-org work (also noted in [api-quirks.md](./api-quirks.md)).
...
|-----|------|----------------------|
| live verification org | 2026-05 (quirks last reviewed 2026-05-12) | Contacts list is a bare array; `ordering=-field` is descending; `ordering=-id` ignored; no contact timestamps; `@os.code` emails; events are org-level (`/projects/:id/events/` → 404); `events_data` vs `/events/:id/` field split; `design` gzip+base64; `integration_json` Python-literal; `integration_key_*` Fernet |
| live verification org | 2026-09-22 | `GET /orgs/<live-org>/events/{id}/` returned the event whose id came from project `events_data` (HTTP success, matching `id`). `GET /orgs/<live-org>/contacts/?ordering=-family_name` was descending by `family_name`. `ordering=-id` produced the same id sequence as the default contacts list (ignored). |
| live verification org | 2026-09-22 | `POST /orgs/<live-org>/contacts/` with `first_name`, `family_name`, `email`, and `phone` was accepted. No other fields were sent. No response body stored. |
| live verification org | 2026-09-22 | `PUT /orgs/<live-org>/contacts/{id}/` with `first_name`, `family_name`, `email`, and `phone` was accepted. The contact was a fixture created for this check. No other fields were sent. No response body stored. |
| live verification org | 2026-09-22 | One fixture contact was created, updated, and deleted (`POST`, `PUT`, then `DELETE` `/orgs/<live-org>/contacts/{id}/`) using `first_name`, `family_name`, `email`, and `phone`. No response body stored. |
| live verification org | 2026-09-22 | `POST /orgs/<live-org>/projects/<fixture-project>/generate_document/proposal/?action=save` returned a private file id. The pdf and docx paths were not called. No response body stored. |
| live verification org | 2026-09-22 | `GET /orgs/<live-org>/projects/<fixture-project>/systems/<fixture-system-uuid>/image/?width=500&height=500` returned `image/jpeg`. The final URL and headers did not contain `/private_files/{id}`. A following private-files list for that project included a new file tagged System Image. `include_contents` was false. No image bytes stored. |
| live verification org | 2026-09-22 | A disposable contact received a PUT that sent only a changed `phone`. The following GET kept `first_name`, `family_name`, and `email`. The fixture was deleted. No response body stored. |
| live verification org | 2026-09-22 | MCP `search_contacts` and `search_projects` returned a listed record id. Scan metadata matched the pages read. An incomplete scan was not treated as absence. No response body stored. |
| live verification org | 2026-09-22 | MCP `get_project_snapshot` returned the project id from the documented project read. A forced workflow failure became a gap and left the project id in place. The output had no design blob, signed file URL, `configuration_json`, or credential field. No response body stored. |
| live verification org | 2026-09-22 | MCP `compare_project_systems` returned system ids from the systems list for one project. The details call ran once only when a listed system was missing a hardware group. No response body stored. |
| live verification org | 2026-09-22 | MCP `get_project_design` ran because this org has Raw Data. Summary matched the decoded structure. `components` and `energy` stayed `unmapped: true`. Geometry did not return coordinates. The raw design payload was not stored. |
...
| Private file `page` and `limit` | The private files query table (retrieved 2026-09-22) does not list `page` or `limit`. `list_private_files` sends both because P5-01 requires a bounded page. A live list on a live verification org the same day sent `project`, `page`, `limit`, and `ordering=-created_date` and received a page. |
...
| Generate document pdf and docx bodies | The html `generate_document` path returned an id on a live verification org on 2026-09-22. The pdf and docx paths were not called. The page still shows no response JSON. |
| System image contents | The 500×500 call on a live verification org on 2026-09-22 did not download bytes. `include_contents` and the 10 MB refusal were not live-checked. |
...
| System comparison | `compare_project_systems` uses the documented systems list and, only when a listed system is missing one or more requested hardware groups, system details with `include_parts=modules,inverters,batteries`. A live verification org called the MCP tool on 2026-09-22. Returned ids matched that project's systems list, and the details call ran once only when a group was missing. No response body was stored. |
| Project share preflight | `preflight_project_share` was not called on a live org. The live suite skips it unless `OPENSOLAR_TEST_CONNECTED_ORG_ID` is set, and it does not create a connection. The sharing-a-project page shows `shared_with` with `org_id` and optional `is_shared`. The write default of `is_shared` is not used as a read contract. The connected-org schema names `is_active` (enabled by this org), `is_other_active` (accepted by the partner), and `is_other_enabled` (active on the partner's side). Disabling a connection documents that `is_active: false` stops sharing both ways. Accessing shared projects and entities (retrieved 2026-09-22) says that with `fieldset=list`, other entities can be filtered by `shared_with` (org id). The payment option, module activation, and systems list examples still do not embed that share list. The bulk share page says unshared entities add warnings to project data and does not name the warning key. Inverter, battery, and other activation id keys are not in the systems list example. No response body was stored. |
| Project stage PATCH | The projects page documents the workflow PATCH body. `update_project_stage` was not called on a live org. The live suite runs that mutation only when `OPENSOLAR_INTEGRATION_WRITES=1` and `OPENSOLAR_TEST_PROJECT_ID` name a dedicated fixture project. |
...
| Project list fields used by `search_projects` | Projects page retrieved 2026-09-22. The list example includes `title`, `address`, `business_name`, and `contacts_data` with `email`, `phone`, `first_name`, `family_name`, and `display`. Project `identifier`, `locality`, `state`, and `zip` are on the detail example, not the list example. A live verification org called the MCP tool the same day and a listed project id was returned. No response body was stored. |
| Project snapshot fields | The same projects page shows detail fields `payment_option_sold`, `private_files_data`, `shared_with`, `usage`, `events_data`, and `assigned_role_data`. `get_project_snapshot` uses those. A live verification org called the MCP tool on 2026-09-22. The project id matched, and a forced workflow failure became a gap. No response body was stored. |
| Nested `GET /projects/:project_id/systems/` | Systems docs specify `GET /systems/?fieldset=list&project=` |
| Conventions doc claim that `-` means ascending | Contradicted by Private Files docs and live-verification-org contacts |
