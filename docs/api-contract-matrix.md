...
| `get_project_snapshot` | composite | documented | GET | `/api/orgs/:org_id/projects/:id/`, `/workflows/:id/`, `/systems/?fieldset=list&project=`, `/payment_options/:id/` when `payment_option_sold` is set, `/private_files/?project=` when `private_files_data` is absent | systems: `fieldset=list`, `project`, `page`, `limit`. Files list: `project`, `page`, `limit`, `ordering=-created_date` | none | n/a | api_access | project detail 100/min user, 10,000/day org; private files 1000/day org | `documented` names those upstream reads. OpenSolar does not document this MCP tool. A failed section is `{ gap }`, not an empty list. Design, file URLs, and `configuration_json` are omitted. `payment_option` is loaded only from `payment_option_sold`. After the project payload is parsed, the workflow, sold payment option, and private-files fallback run together. Usage keeps the documented period: annual and additive period sources include `annual_total`; `kwh_daily_per_month` is not summed. File metadata is `returned_count`, `total_count`, and `list_complete`. Embedded `private_files_data` and a short private-files page set `total_count`. A full five-row page sets `total_count` to null. Contacts include `display` from the project payload. `shared_with` is copied from the project payload. Not live-verified. | [Projects](https://developers.opensolar.com/api/projects/), [Systems](https://developers.opensolar.com/api/system/), [Private Files](https://developers.opensolar.com/api/private-files/) |
...
| `update_project_stage` | safe_wrapper | documented | PATCH | `/api/orgs/:org_id/projects/:id/` | none | none | See [Workflow stage replacement](#workflow-stage-replacement). Do **not** PATCH deprecated `stage`. | api_access | 10/min user, 10,000/day org | Send exactly one of `active_stage_id` or `stage_name`. `workflow_id` is required with `active_stage_id`. `stage_name` reads the project when `workflow_id` is omitted, then the workflow, and PATCHes only when one stage title matches. No match and two matches do not PATCH. Not live-verified. | [Projects](https://developers.opensolar.com/api/projects/) |
...
| `preflight_project_share` | derived | documented | GET | `/api/orgs/:org_id/connected_orgs/`, `/api/orgs/:org_id/projects/:id/`, `/api/orgs/:org_id/systems/` | connected orgs: `fieldset=list`, `page`, `limit=100`, at most 3 pages. Systems: `fieldset=list`, `project`, `page=1`, `limit=100` | list on both collections | n/a | api_access | connected orgs 100/day user and org | `documented` names those upstream reads. OpenSolar does not document this MCP tool. It does not call `share_project` or `share_entities`. `connection` is `active` only when the scan finishes and one match has `is_active` true. An unfinished scan is `unknown`. `project_share` uses `shared_with`. Payment option, pricing scheme, costing, and module activation ids are copied when the documented payload has them. Their `share` stays `unknown` because those reads do not list which orgs an entity is shared with. Inverter, battery, and other activation ids stay `unknown` because the systems list example does not name those keys. `unknown` is not treated as safe. Not live-verified. | [Listing established connections](https://developers.opensolar.com/api/listing-established-connections/), [Sharing a project](https://developers.opensolar.com/api/sharing-a-project/), [Projects](https://developers.opensolar.com/api/projects/), [Systems](https://developers.opensolar.com/api/systems/), [source-log.md](./source-log.md) |
...
| MCP `search_contacts` | derived | documented | Pages `GET /api/orgs/:org_id/contacts/` with `page` and `limit` and matches locally. Status becomes `live-verified` only after a sandbox check. |
