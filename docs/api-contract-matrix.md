...
| `list_workflows` |  | documented | GET | `/api/orgs/:org_id/workflows/` | `page`, `limit`, `is_default`, `is_archived` | none | n/a | api_access | undocumented quota | Bare array. MCP list sends `page` and `limit` only. Curated stages are `id`, `title`, `milestone`, `order`, and `is_archived`. Actions are omitted. | [Workflows](https://developers.opensolar.com/api/workflows/) |
...
| `list_private_files` |  | documented | GET | `/api/orgs/:org_id/private_files/` | documented: `project`, `user_id`, `file_tags`, `file_tags_exclude`, `search`, `ordering`. Tool also sends `page` and `limit` (not on the query table; retrieved 2026-09-22). | none | n/a | api_access | 1000/day org, 1000/hour user | Bare array. MCP output is `{ private_files, page, limit }`. Curated row: `id`, `title`, file-tag titles, `project_id`. `file_contents` is omitted. Optional filters are sent only when set. `project_id` is sent as `project`. The documented filter set is covered by the existing list tool; no separate file-search tool is required. | [Private Files](https://developers.opensolar.com/api/private-files/), [source-log.md](./source-log.md) |
| `get_private_file` |  | documented | GET | `/api/orgs/:org_id/private_files/:file_id/` | none | none | n/a | api_access | 1000/day org, 1000/hour user | One curated file plus `size` (`filesize`) and `content_type` when present. `include_contents: true` downloads `file_contents` on the server. Structured contents are text only; text is capped at 100,000 characters and sets `truncated`. Images use MCP image blocks. PDFs include extracted text and an `application/pdf` resource at `opensolar://private-files/{id}`; other binary files use embedded resources. Binary bytes are not duplicated into structured output. Bodies over 10 MB are refused. The download URL is not returned. Not live-verified. | [Private Files](https://developers.opensolar.com/api/private-files/) |
| `create_private_file` |  | documented | POST | `/api/orgs/:org_id/private_files/` | none | none | multipart `title`, `file_contents` | api_access | 1000/day org, 1000/hour user | MCP tool takes a filesystem path and `title`. Local uploads are disabled unless `OPENSOLAR_UPLOAD_ROOT` is configured. The server resolves both the root and file real paths and refuses any path outside the root, including symlink escapes, before streaming the file. The create sample does not send `project` or `file_tags`, so those fields are not sent. Result is the curated file. The download URL is omitted. Not live-verified. | [Private Files](https://developers.opensolar.com/api/private-files/), [source-log.md](./source-log.md) |
| `update_private_file` |  | documented | PATCH | `/api/orgs/:org_id/private_files/:file_id/` | none | none | `{ title }` | api_access | 1000/day org, 1000/hour user | The published sample sends only `title`. Result is the curated file. The download URL is omitted. Not live-verified. | [Private Files](https://developers.opensolar.com/api/private-files/) |
| `delete_private_file` |  | documented | DELETE | `/api/orgs/:org_id/private_files/:file_id/` | none | none | none | api_access | 1000/day org, 1000/hour user | Input is `id`. Result is `{ id, deleted: true }`. The upstream body is not returned. Not live-verified. | [Private Files](https://developers.opensolar.com/api/private-files/) |
| `generate_project_document` |  | documented | POST | `/api/orgs/:org_id/projects/:project_id/generate_document/:document_type/` (recommended) and `generate_document_docx`. The legacy `generate_document_pdf` is not used | `action=save`; `file_format=pdf` or `csv` when `format` is set | none | no JSON body | api_access | undocumented quota | Omitted `format` calls `generate_document` without `file_format`, so OpenSolar uses the document type's default (PDF for most types, CSV for some). `pdf` and `csv` set the documented `file_format` on `generate_document`; `docx` calls `generate_document_docx`. The docs do not describe HTML output; the earlier `html` option was renamed to the omitted-format default on 2026-09-23. The tool sends `action=save` and returns `{ id }`. File bytes are not returned. The page shows no response body. Live-checked 2026-09-22 on a live verification org: a proposal through `generate_document` without `file_format` (then labelled `html`) returned a private file id. `file_format`, pdf, csv, and docx were not called. | [Generating Project Files](https://developers.opensolar.com/api/generating-project-files/), [source-log.md](./source-log.md) |
| `list_webhooks` |  | documented | GET | `/api/orgs/:org_id/webhooks/` | none | none | n/a | api_access | User Terms: 2,000 webhook calls/month unless agreed otherwise | The page shows no response JSON and no page parameters. MCP output is `{ webhooks }`. Curated row: `id`, `endpoint`, `enabled`, `debug`, `trigger_fields`, `payload_fields`. `headers` is omitted. Not live-verified. | [Webhooks](https://developers.opensolar.com/api/webhooks/), [source-log.md](./source-log.md) |
| `create_webhook` |  | documented | POST | `/api/orgs/:org_id/webhooks/` | none | none | `endpoint`, `enabled`, `debug`; optional `trigger_fields`, `payload_fields` | api_access | User Terms: 2,000 webhook calls/month unless agreed otherwise | Required fields match the parameters table. Unset field lists are omitted so OpenSolar applies its defaults. The create curl also sends `headers`; the tool does not. Result is the curated webhook. Not live-verified. | [Webhooks](https://developers.opensolar.com/api/webhooks/), [source-log.md](./source-log.md) |
| `update_webhook` |  | documented | PATCH | `/api/orgs/:org_id/webhooks/:id/` | none | none | any of `endpoint`, `enabled`, `debug`, `trigger_fields`, `payload_fields` | api_access | User Terms: 2,000 webhook calls/month unless agreed otherwise | The sample sends `enabled`, `trigger_fields`, and `payload_fields`. The parameters table also lists `endpoint` and `debug`, so those are accepted. At least one field besides id. A blank field list on an existing webhook is treated as null. No delete tool. Not live-verified. | [Webhooks](https://developers.opensolar.com/api/webhooks/), [source-log.md](./source-log.md) |
| `list_webhook_logs` |  | documented | GET | `/api/orgs/:org_id/webhook_process_logs/` | `page`, `limit`, optional `search` | none | n/a | api_access | User Terms: 2,000 webhook calls/month unless agreed otherwise | Bare array. MCP output is `{ webhook_logs, page, limit }`. Curated row: `id`, `webhook_id`, `event_queue_name`, `created_date`, `modified_date`, `event_timestamp`. An empty page can return HTTP 500. Delivery notes are omitted. `format=csv` is not sent. Not live-verified. | [Webhooks Logs](https://developers.opensolar.com/api/webhooks-logs/) |
| `list_webhook_queue` |  | documented | GET | `/api/orgs/:org_id/webhook_queue_models/` | tool sends `page` and `limit` (not on the page; retrieved 2026-09-22) | none | n/a | api_access | User Terms: 2,000 webhook calls/month unless agreed otherwise | Bare array. MCP output is `{ webhook_queue, page, limit }`. Curated row: `id`, `webhook_id`, `model_name`, `event`, `number_of_attempts`, `next_attempt_at`, `processing_started_at`. `model_pk` and `foreign_identifier` are omitted. Queue rows have no `event_queue_name`; that field is on log rows only. `format=csv` is not sent. Not live-verified. | [Webhooks Queue](https://developers.opensolar.com/api/webhooks-queue/), [source-log.md](./source-log.md) |
...
| `delete_connection` |  | documented | DELETE | `/api/orgs/:org_id/connected_orgs/:id/` | none | none | none | api_access | undocumented quota (the throttle page lists only create, update, and list connected-org calls) | Result is `{ id, deleted: true }`. The upstream body is not returned. Not live-verified. | [Deleting a connection](https://developers.opensolar.com/api/deleting-a-connection/) |
| `share_project` |  | documented | PUT | `/api/orgs/:org_id/projects/:id/` | none | none | `shared_with` (`org_id`, `permission`, optional `is_shared`) | api_access | 10/min user, 10,000/day org (Update Project) | One org per call. `permission` is built from `permission_role_id`. Omitted `is_shared` is left off the body. The project body is not returned. The docs sample sends one `shared_with` entry and does not say whether other existing shares are kept; confirm with a dedicated fixture before sharing one project with several orgs. Not live-verified. | [Sharing a Project](https://developers.opensolar.com/api/sharing-a-project/) |
...
| Bulk project operations | deferred | — | none | — | — | — | — | create is 10/min | OpenSolar has no documented batch project API. Bulk mutation is therefore not exposed as a single tool. | [Schema overview](https://developers.opensolar.com/api/schema-overview/), [Throttle](https://developers.opensolar.com/api/throttle/) |
...
`list_workflows` supplies `workflow_stages[].id` and titles. `stage_name` resolves a title on the workflow before PATCH. Archived stages (`is_archived: true` on a workflow stage) are never matched by name. Exactly one title may match. Do not send `stage: <n>` for this tool.
