...
| unsupported | No usable published request. Do not send it |
...
| `search_projects` | derived | documented | GET | `/api/orgs/:org_id/projects/` | `page`, `limit` only. No `search`, `fieldset`, or `verbose`. | none | n/a | api_access | undocumented quota | Local match over the documented project list. The official list example includes `title`, `address`, `business_name`, and `contacts_data` name, email, and phone. `identifier`, `locality`, `state`, and `zip` match only when that list row carries them. The tool does not call `get_project`. Page cap 20 and result cap 25 are MCP work bounds. `complete` is true only when the last fetched page is short. `results_truncated` is true only when scanned pages held more matches than were returned. Not live-verified. | [Projects](https://developers.opensolar.com/api/projects/) |
...
| `search_contacts` | derived | documented | GET | `/api/orgs/:org_id/contacts/` | `page`, `limit` only. No `search` and no `ordering`. | none | n/a | api_access | undocumented quota | Local match over the documented contact list. Name, email, and phone. Passport, licence, and date of birth are redacted before matching. Page cap 20 and result cap 25 are MCP work bounds. `complete` is true only when the last fetched page is short. `results_truncated` is true only when scanned pages held more matches than were returned. Not live-verified. | [Contacts](https://developers.opensolar.com/api/contacts/) |
