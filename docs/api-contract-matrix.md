...
| `get_project_design` | derived | live-verified | GET | `/api/orgs/:org_id/projects/:id/` | none | none | n/a | raw_data | 100/min user, 10,000/day org | Reads `design`. Missing or null returns `design_available: false`. A string is gzip+base64. `section` defaults to `summary`: `system_count` and `system_price_including_tax`. `components` and `energy` return `unmapped: true` because the decompress section does not name those keys. `geometry` reports whether `autoFacetsGeoJson` is present and does not return coordinates. `financials` copies finite numbers for the pricing keys named on each system object and does not copy objects or arrays. The compressed string is not returned. HTTP 402 maps to the Raw Data message. `get_project` verbose still replaces `design` with `[REDACTED]`. Live-checked 2026-09-22 on org 48389 through the MCP tool when Raw Data was present. Summary matched the decoded structure. `components` and `energy` stayed `unmapped: true`. Geometry did not return coordinates. | [Projects](https://developers.opensolar.com/api/projects/), [source-log.md](./source-log.md) |
...
| MCP `search_contacts` | derived | live-verified | Pages `GET /api/orgs/:org_id/contacts/` with `page` and `limit` and matches locally. Org 48389 returned a listed contact id through the MCP tool on 2026-09-22. |
