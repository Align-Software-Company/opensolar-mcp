...
export type EventTypeName = (typeof EVENT_TYPES)[EventTypeId];

export function getEventTypeName(id: number): string {
  const lookup = EVENT_TYPES as Record<number, string | undefined>;
  return lookup[id] ?? 'Unknown event type';
}
